...
class Cfg:
    ...
    # 输出目录
    out_dir: str = "BottleneckDiagnostics/Diagnostic2_UNetVolume/LayeredSplit_TargetedCoverage_BCEOnly"

    # 固定单域 5-shot 训练病人，剩余先作为 held-out，再分 inner / hard-outer
    ...
    # -------- segmentation loss: BCE only (dice_loss still monitored but not in total loss) --------
    ...
    # 控制体量：train / inner / hard-outer 默认只输出指标与CSV；test 全量导出可视化
    save_all_eval_vis: bool = False
    ...
    # -------- focused variants: U-Net family scale sweep --------
    variants: Tuple[Variant, ...] = (
        # Depth = 2
        Variant(name="unet_d2_b8_l16_wd1e-4",  model_type="unet", depth=2, base_ch=8,  latent_ch=16,  weight_decay=1e-4),
        Variant(name="unet_d2_b8_l32_wd1e-4",  model_type="unet", depth=2, base_ch=8,  latent_ch=32,  weight_decay=1e-4),
        Variant(name="unet_d2_b12_l24_wd1e-4", model_type="unet", depth=2, base_ch=12, latent_ch=24,  weight_decay=1e-4),
        Variant(name="unet_d2_b12_l48_wd1e-4", model_type="unet", depth=2, base_ch=12, latent_ch=48,  weight_decay=1e-4),
        # Depth = 3
        Variant(name="unet_d3_b8_l32_wd1e-4",  model_type="unet", depth=3, base_ch=8,  latent_ch=32,  weight_decay=1e-4),
        Variant(name="unet_d3_b8_l64_wd1e-4",  model_type="unet", depth=3, base_ch=8,  latent_ch=64,  weight_decay=1e-4),
        Variant(name="unet_d3_b12_l48_wd1e-4", model_type="unet", depth=3, base_ch=12, latent_ch=48,  weight_decay=1e-4),
        Variant(name="unet_d3_b16_l64_wd1e-4", model_type="unet", depth=3, base_ch=16, latent_ch=64,  weight_decay=1e-4),
        # Depth = 4
        Variant(name="unet_d4_b8_l64_wd1e-4",  model_type="unet", depth=4, base_ch=8,  latent_ch=64,  weight_decay=1e-4),
        Variant(name="unet_d4_b12_l64_wd1e-4", model_type="unet", depth=4, base_ch=12, latent_ch=64,  weight_decay=1e-4),
        Variant(name="unet_d4_b12_l96_wd1e-4", model_type="unet", depth=4, base_ch=12, latent_ch=96,  weight_decay=1e-4),
        Variant(name="unet_d4_b16_l128_wd1e-4",model_type="unet", depth=4, base_ch=16, latent_ch=128, weight_decay=1e-4),
    )
...
class GenericUNet(BaseSegNet):
    """Variable-depth U-Net. depth >= 2, arbitrary base_ch and latent_ch."""

    def __init__(self, depth: int = 3, base_ch: int = 16, latent_ch: int = 64):
        super().__init__(model_type="unet")
        assert depth >= 2, f"GenericUNet requires depth >= 2, got {depth}"
        self.depth = depth
        ...
        # Bottleneck
        self.bottleneck_block = ConvBlock(enc_channels[-1], latent_ch)
        ...
        for i in range(depth - 1, -1, -1):
            ...
            if i == depth - 1:
                ...
                up_out_ch = skip_ch
            self.ups.append(nn.ConvTranspose2d(dec_in, up_out_ch, kernel_size=2, stride=2))
            dec_out_ch = enc_channels[i - 1] if i > 0 else base_ch
            self.decoders.append(ConvBlock(up_out_ch + skip_ch, dec_out_ch))
            self.dec_out_channels.append(dec_out_ch)
            dec_in = dec_out_ch

        self.seg_head = nn.Conv2d(self.dec_out_channels[-1], 1, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor, return_features: bool = False):
        ...
        # Bottleneck
        bottleneck = self.bottleneck_block(h)

        # Decoder forward
        dec_outs = []
        h = bottleneck
        for i in range(self.depth):
            enc_idx = self.depth - 1 - i  # reverse order
            h = self.ups[i](h)
            h = torch.cat([h, enc_outs[enc_idx]], dim=1)
            h = self.decoders[i](h)
            dec_outs.append(h)

        logits = self.seg_head(h)

        if return_features:
            feat_dict = {}
            for i, e in enumerate(enc_outs):
                feat_dict[f"enc{i+1}"] = e
            feat_dict["bottleneck"] = bottleneck
            for i, d in enumerate(dec_outs):
                feat_dict[f"dec{self.depth - i}"] = d
            return {
                "logits": logits,
                "features": self.collect_feature_dict(**feat_dict),
            }
        return logits
...
def build_model(variant: Variant) -> nn.Module:
    if variant.model_type == "unet":
        return GenericUNet(depth=variant.depth, base_ch=variant.base_ch, latent_ch=variant.latent_ch)
...
def aggregate_all_variants(root: Path, variant_names: List[str]):
    ...
    important_metrics = [
        ...
        "best_inner_ckpt_test_mean_dice",
        "best_hard_ckpt_inner_mean_dice",
        "best_hard_ckpt_hard_outer_mean_dice",
        "best_hard_ckpt_test_mean_dice",
        "hard_outer_sacrifice_dice_when_select_inner",
        "inner_sacrifice_dice_when_select_hard",
        "gap_train_minus_test_at_best_inner",
        "gap_train_minus_hard_at_best_inner",
        "gap_train_minus_inner_at_best_inner",
        "best_inner_epoch",
        "best_hard_epoch",
        "stop_epoch",
        "params_total",
    ...
    with open(root / "global_report.txt", "w", encoding="utf-8") as f:
        f.write("U-Net family scale sweep (BCE only): layered split model-selection + anti-overfitting controls\n")
...
# =========================================================
# Demo Baseline Selection
# =========================================================
def select_demo_baseline(root: Path):
    """Auto-select best demo baseline from all_variants_seed_results.csv."""
    csv_path = root / "all_variants_seed_results.csv"
    rows = maybe_read_csv_rows(csv_path)
    if not rows:
        print("[select_demo_baseline] No data found in all_variants_seed_results.csv")
        return
    ...
    variant_stats = []
    for vname, vrows in grouped.items():
        params_totals = [safe_float(r.get("params_total")) for r in vrows]
        best_inner_test_dices = [safe_float(r.get("best_inner_ckpt_test_mean_dice")) for r in vrows]
        best_inner_hard_outer_dices = [safe_float(r.get("best_inner_ckpt_hard_outer_mean_dice")) for r in vrows]
        best_inner_inner_dices = [safe_float(r.get("best_inner_ckpt_inner_mean_dice")) for r in vrows]
        last_test_dices = [safe_float(r.get("last_test_mean_dice")) for r in vrows]
        gap_train_test = [safe_float(r.get("gap_train_minus_test_at_best_inner")) for r in vrows]

        best_inner_test_mean = float(np.mean(best_inner_test_dices))
        best_inner_test_std = float(np.std(best_inner_test_dices))
        last_test_mean = float(np.mean(last_test_dices))
        endpoint_drop = best_inner_test_mean - last_test_mean

        # Get depth from rows
        depth = safe_float(vrows[0].get("depth", 0))

        variant_stats.append({
            "variant": vname,
            "depth": int(depth),
            "base_ch": int(safe_float(vrows[0].get("base_ch", 0))),
            "latent_ch": int(safe_float(vrows[0].get("latent_ch", 0))),
            "params_total_mean": float(np.mean(params_totals)),
            "best_inner_test_mean": best_inner_test_mean,
            "best_inner_test_std": best_inner_test_std,
            "best_inner_hard_outer_mean": float(np.mean(best_inner_hard_outer_dices)),
            "best_inner_inner_mean": float(np.mean(best_inner_inner_dices)),
            "last_test_mean": last_test_mean,
            "endpoint_drop": endpoint_drop,
            "train_test_gap": float(np.mean(gap_train_test)),
            "inner_hard_gap": float(np.mean(best_inner_inner_dices)) - float(np.mean(best_inner_hard_outer_dices)),
            "n_seeds": len(vrows),
        })

    # Write full candidates csv
    write_log_csv(variant_stats, root / "demo_baseline_candidates.csv")

    # Selection logic
    max_best_inner_test = max(s["best_inner_test_mean"] for s in variant_stats)

    # Step 1: performance not too far from best
    candidates = [s for s in variant_stats if s["best_inner_test_mean"] >= max_best_inner_test - 0.03]
    step1_names = {s["variant"] for s in candidates}

    # Step 2: stability
    candidates = [s for s in candidates if s["best_inner_test_std"] <= 0.03]
    step2_names = {s["variant"] for s in candidates}

    # Step 3: not excessive train-test gap
    candidates = [s for s in candidates if s["train_test_gap"] <= 0.18]
    step3_names = {s["variant"] for s in candidates}

    # Step 4: sort by (hard_outer high, endpoint_drop small, params small)
    if candidates:
        candidates.sort(key=lambda s: (-s["best_inner_hard_outer_mean"], s["endpoint_drop"], s["params_total_mean"]))
        selected = candidates[0]
    else:
        # Fallback: pick the one with highest best_inner_test_mean
        variant_stats_sorted = sorted(variant_stats, key=lambda s: -s["best_inner_test_mean"])
        selected = variant_stats_sorted[0]
        candidates = variant_stats_sorted[:1]

    # Write report
    with open(root / "demo_baseline_report.txt", "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("DEMO BASELINE SELECTION REPORT\n")
        f.write("=" * 80 + "\n\n")

        f.write("Selection rules:\n")
        f.write(f"  Step 1: best_inner_test_mean >= {max_best_inner_test:.4f} - 0.03 = {max_best_inner_test - 0.03:.4f}\n")
        f.write(f"          Passed: {sorted(step1_names)}\n")
        f.write(f"  Step 2: best_inner_test_std <= 0.03\n")
        f.write(f"          Passed: {sorted(step2_names)}\n")
        f.write(f"  Step 3: train_test_gap <= 0.18\n")
        f.write(f"          Passed: {sorted(step3_names)}\n")
        f.write(f"  Step 4: Sort by hard_outer_mean (desc), endpoint_drop (asc), params (asc)\n\n")

        f.write("Candidate ranking after all filters:\n")
        f.write("-" * 80 + "\n")
        for i, c in enumerate(candidates):
            f.write(
                f"  Rank {i+1}: {c['variant']}\n"
                f"    depth={c['depth']} base_ch={c['base_ch']} latent_ch={c['latent_ch']}\n"
                f"    params_total={c['params_total_mean']:.0f}\n"
                f"    best_inner_test_mean={c['best_inner_test_mean']:.4f} (std={c['best_inner_test_std']:.4f})\n"
                f"    best_inner_hard_outer_mean={c['best_inner_hard_outer_mean']:.4f}\n"
                f"    best_inner_inner_mean={c['best_inner_inner_mean']:.4f}\n"
                f"    last_test_mean={c['last_test_mean']:.4f}\n"
                f"    endpoint_drop={c['endpoint_drop']:.4f}\n"
                f"    train_test_gap={c['train_test_gap']:.4f}\n"
                f"    inner_hard_gap={c['inner_hard_gap']:.4f}\n\n"
            )

        f.write("=" * 80 + "\n")
        f.write(f"SELECTED DEMO BASELINE: {selected['variant']}\n")
        f.write("=" * 80 + "\n")
        f.write(f"  depth           = {selected['depth']}\n")
        f.write(f"  base_ch         = {selected['base_ch']}\n")
        f.write(f"  latent_ch       = {selected['latent_ch']}\n")
        f.write(f"  params_total    = {selected['params_total_mean']:.0f}\n")
        f.write(f"  best_inner_test = {selected['best_inner_test_mean']:.4f} +/- {selected['best_inner_test_std']:.4f}\n")
        f.write(f"  hard_outer      = {selected['best_inner_hard_outer_mean']:.4f}\n")
        f.write(f"  inner           = {selected['best_inner_inner_mean']:.4f}\n")
        f.write(f"  last_test       = {selected['last_test_mean']:.4f}\n")
        f.write(f"  endpoint_drop   = {selected['endpoint_drop']:.4f}\n")
        f.write(f"  train_test_gap  = {selected['train_test_gap']:.4f}\n")
        f.write(f"  inner_hard_gap  = {selected['inner_hard_gap']:.4f}\n")

    print(f"[Demo Baseline] Selected: {selected['variant']}")


# =========================================================
# Scale Sweep Report
# =========================================================
def generate_scale_sweep_report(root: Path):
    """Generate scale_sweep_report.txt summarizing capacity sweep results."""
    csv_path = root / "all_variants_seed_results.csv"
    rows = maybe_read_csv_rows(csv_path)
    if not rows:
        return
    ...
    vstats = []
    for vname, vrows in grouped.items():
        depth = int(safe_float(vrows[0].get("depth", 0)))
        base_ch = int(safe_float(vrows[0].get("base_ch", 0)))
        latent_ch = int(safe_float(vrows[0].get("latent_ch", 0)))
        params = float(np.mean([safe_float(r.get("params_total")) for r in vrows]))
        bi_test = float(np.mean([safe_float(r.get("best_inner_ckpt_test_mean_dice")) for r in vrows]))
        bi_test_std = float(np.std([safe_float(r.get("best_inner_ckpt_test_mean_dice")) for r in vrows]))
        bi_inner = float(np.mean([safe_float(r.get("best_inner_ckpt_inner_mean_dice")) for r in vrows]))
        bi_hard = float(np.mean([safe_float(r.get("best_inner_ckpt_hard_outer_mean_dice")) for r in vrows]))
        last_test = float(np.mean([safe_float(r.get("last_test_mean_dice")) for r in vrows]))
        gap_tt = float(np.mean([safe_float(r.get("gap_train_minus_test_at_best_inner")) for r in vrows]))
        ...
        vstats.append({
            ...
            "params": params,
            "bi_test": bi_test,
            "bi_test_std": bi_test_std,
            "bi_inner": bi_inner,
            "bi_hard": bi_hard,
            "last_test": last_test,
            "endpoint_drop": bi_test - last_test,
            "gap_tt": gap_tt,
        })

    vstats.sort(key=lambda s: s["params"])
    ...
    with open(root / "scale_sweep_report.txt", "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("U-NET FAMILY SCALE SWEEP REPORT (BCE ONLY)\n")
        f.write("=" * 80 + "\n\n")

        # Overall summary
        smallest = vstats[0]
        largest = vstats[-1]
        middle_idx = len(vstats) // 2
        middle = vstats[middle_idx]

        f.write("--- Size comparison: smallest / middle / largest ---\n")
        for label, s in [("Smallest", smallest), ("Middle", middle), ("Largest", largest)]:
            f.write(
                f"  {label}: {s['variant']}  params={s['params']:.0f}  "
                f"bi_test={s['bi_test']:.4f}  bi_hard={s['bi_hard']:.4f}  "
                f"endpoint_drop={s['endpoint_drop']:.4f}\n"
            )
        f.write("\n")
        ...
        for depth in sorted(by_depth.keys()):
            ...
            group.sort(key=lambda s: -s["bi_test"])
            best_in_group = group[0]
            avg_bi_test = float(np.mean([s["bi_test"] for s in group]))
            avg_bi_hard = float(np.mean([s["bi_hard"] for s in group]))
            avg_params = float(np.mean([s["params"] for s in group]))

            f.write(f"--- Depth = {depth} ({len(group)} variants) ---\n")
            f.write(f"  Group avg: bi_test={avg_bi_test:.4f}  bi_hard={avg_bi_hard:.4f}  params={avg_params:.0f}\n")
            f.write(f"  Best in group: {best_in_group['variant']}  bi_test={best_in_group['bi_test']:.4f}\n")
            for s in group:
                f.write(
                    f"    {s['variant']:35s} params={s['params']:8.0f}  "
                    f"bi_test={s['bi_test']:.4f}+/-{s['bi_test_std']:.4f}  "
                    f"bi_inner={s['bi_inner']:.4f}  bi_hard={s['bi_hard']:.4f}  "
                    f"last_test={s['last_test']:.4f}  drop={s['endpoint_drop']:.4f}  "
                    f"gap_tt={s['gap_tt']:.4f}\n"
                )
            f.write("\n")

        # Diagnostic observations
        f.write("--- Diagnostic observations ---\n")

        # Check capacity bottleneck: if smallest models have much lower bi_test
        overall_best_test = max(s["bi_test"] for s in vstats)
        bottleneck_variants = [s for s in vstats if s["bi_test"] < overall_best_test - 0.05]
        if bottleneck_variants:
            f.write(f"  Capacity bottleneck detected in: {[s['variant'] for s in bottleneck_variants]}\n")
            f.write(f"    These are >0.05 below best bi_test ({overall_best_test:.4f})\n")
        else:
            f.write("  No clear capacity bottleneck detected (all within 0.05 of best).\n")

        # Check deeper-but-less-stable
        depth_best = {}
        for depth in sorted(by_depth.keys()):
            group = by_depth[depth]
            depth_best[depth] = max(s["bi_test"] for s in group)

        depths_sorted = sorted(depth_best.keys())
        deeper_unstable = False
        for i in range(1, len(depths_sorted)):
            d_prev = depths_sorted[i - 1]
            d_curr = depths_sorted[i]
            avg_drop_prev = float(np.mean([s["endpoint_drop"] for s in by_depth[d_prev]]))
            avg_drop_curr = float(np.mean([s["endpoint_drop"] for s in by_depth[d_curr]]))
            if avg_drop_curr > avg_drop_prev + 0.01:
                f.write(
                    f"  Deeper-but-less-stable: depth={d_curr} avg endpoint_drop={avg_drop_curr:.4f} "
                    f"> depth={d_prev} avg endpoint_drop={avg_drop_prev:.4f}\n"
                )
                deeper_unstable = True
        if not deeper_unstable:
            f.write("  No deeper-but-less-stable trend detected.\n")

    print(f"[Scale Sweep Report] Written to {root / 'scale_sweep_report.txt'}")
...
def main():
    ...
    print("=" * 100)
    print("U-Net family scale sweep with layered-validation and targeted coverage analysis (BCE only)")
    ...
    for variant in cfg.variants:
        ...
        with open(variant_root / "run_report.txt", "w", encoding="utf-8") as f:
            f.write("U-Net family scale sweep experiment (BCE only)\n")
            f.write(f"Variant             : {variant.name}\n")
            f.write(f"Model type          : {variant.model_type}\n")
            f.write(f"Depth               : {variant.depth}\n")
            ...
            f.write(f"Loss                : BCE only (lambda_bce={cfg.lambda_bce}, lambda_dice={cfg.lambda_dice})\n")
            f.write(f"LR                  : {cfg.lr}\n")
    ...
    with open(Path(cfg.out_dir) / "experiment_manifest.txt", "w", encoding="utf-8") as f:
        f.write("U-Net family scale sweep experiment (BCE only)\n")
        f.write(f"Optimizer: AdamW | LR: {cfg.lr} | Epochs: {cfg.epochs} | Batch size: {cfg.batch_size}\n")
        f.write(f"Default weight_decay: 1e-4 | Loss: BCE only (lambda_dice=0.0)\n")
        f.write(f"num_middle_slices: {cfg.num_middle_slices}\n\n")
        for v in cfg.variants:
            f.write(
                f"{v.name}: model_type={v.model_type}, depth={v.depth}, base_ch={v.base_ch}, latent_ch={v.latent_ch}, "
                f"wd={v.weight_decay}, early_stopping={v.use_early_stopping}, patience={v.es_patience}\n"
            )

    # Auto-select demo baseline and generate scale sweep report
    select_demo_baseline(Path(cfg.out_dir))
    generate_scale_sweep_report(Path(cfg.out_dir))

    print("=" * 100)
    print("U-Net family scale sweep experiments finished (BCE only).")
