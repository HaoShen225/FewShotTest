...
class Cfg:
    ...
    # -------- focused variants: loss ablation (BCE only vs BCE+Dice) --------
    variants: Tuple[Variant, ...] = (
        Variant(
            name="unet_b16_l64_wd1e-4_bce_only",
            model_type="unet",
            base_ch=16,
            latent_ch=64,
            weight_decay=1e-4,
        ...
        Variant(
            name="unet_b16_l64_wd1e-4_bce_plus_dice",
            model_type="unet",
            base_ch=16,
            latent_ch=64,
            weight_decay=1e-4,
            loss_tag="bce_plus_dice",
            lambda_bce=1.0,
            lambda_dice=1.0,
            use_early_stopping=False,
        ),
        Variant(
            name="attention_unet_b16_l64_wd1e-4_bce_only",
        ...
        Variant(
            name="attention_unet_b16_l64_wd1e-4_bce_plus_dice",
            model_type="attention_unet",
            base_ch=16,
            latent_ch=64,
            weight_decay=1e-4,
...
# Train / Eval
# =========================================================
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device, variant: Variant) -> Dict[str, float]:
    model.train()
    ...
    for img, mask, _meta in loader:
        ...
        loss, loss_bce, loss_dice = compute_segmentation_loss(
            seg_logits=seg_logits,
            mask=mask,
            lambda_bce=variant.lambda_bce,
            lambda_dice=variant.lambda_dice,
        )
...
@torch.no_grad()
def evaluate_loader(model: nn.Module, loader: DataLoader, device: torch.device, variant: Variant) -> Dict[str, float]:
    model.eval()
...
def main():
    ...
    for variant in cfg.variants:
        ...
        for seed in cfg.seeds:
            ...
            for epoch in range(cfg.epochs):
                train_stats = train_one_epoch(model, train_loader, optimizer, device, variant)
                train_eval = evaluate_loader(model, train_eval_loader, device, variant)
                inner_eval = evaluate_loader(model, inner_eval_loader, device, variant)
                hard_eval = evaluate_loader(model, hard_outer_eval_loader, device, variant)
                test_eval = evaluate_loader(model, test_eval_loader, device, variant)
        ...
        with open(variant_root / "run_report.txt", "w", encoding="utf-8") as f:
            f.write("3-level U-Net / Attention U-Net layered split experiment (loss ablation)\n")
            ...
            f.write(f"Loss tag            : {variant.loss_tag}\n")
            f.write(f"Lambda BCE          : {variant.lambda_bce}\n")
            f.write(f"Lambda Dice         : {variant.lambda_dice}\n")
            f.write(f"LR                  : {cfg.lr}\n")
    ...
    with open(Path(cfg.out_dir) / "experiment_manifest.txt", "w", encoding="utf-8") as f:
        f.write("3-level U-Net / Attention U-Net loss ablation experiment variants:\n")
        for v in cfg.variants:
            f.write(
                f"{v.name}: model_type={v.model_type}, base_ch={v.base_ch}, latent_ch={v.latent_ch}, "
                f"wd={v.weight_decay}, loss_tag={v.loss_tag}, lambda_bce={v.lambda_bce}, "
                f"lambda_dice={v.lambda_dice}, early_stopping={v.use_early_stopping}, patience={v.es_patience}\n"
            )

    print("=" * 100)
    print("3-level U-Net / Attention U-Net loss ablation experiments finished.")
