...
class Cfg:
    ...
    # -------------------------
    # centerline + 2nd-order legal chain
    # -------------------------
    centerline_poly_deg: int = 2
    centerline_refine_iters: int = 2
    centerline_weight_alpha: float = 2.0
    centerline_dist_penalty: float = 0.06

    # reward terms
    chain_lambda_K: float = 0.35          # +lambda_K per accepted node
    chain_lambda_L: float = 0.012         # +lambda_L * (t_k - t_j), here t=y

    # edge / 2nd-order penalties
    chain_spacing_lambda: float = 0.90    # |dy - d_bar| / d_bar
    chain_gap_lambda: float = 1.20        # G(Δ)=max(0, Δ/d_bar - 1)
    chain_ratio_lambda: float = 0.75      # |log(dy2/dy1)|
    chain_slope_lambda: float = 2.50      # |r2 - r1|

    # fit-centerline candidate filtering
    centerline_keep_topk: int = 20
...
def _polyfit_x_of_y(
    ...
    """
    Fit x = c(y) with weighted least squares.
    coef is in increasing order:
        x = coef[0] + coef[1] * y + coef[2] * y^2 + ...
    """
    n = len(ys)
    if n == 0:
        return np.array([0.0], dtype=np.float64)
    deg = int(min(max(0, deg), max(0, n - 1)))
    V = np.vander(ys, N=deg + 1, increasing=True).astype(np.float64)
    w = np.sqrt(np.clip(ws.astype(np.float64), 1e-8, None))
    A = V * w[:, None]
    b = xs.astype(np.float64) * w
    coef, *_ = np.linalg.lstsq(A, b, rcond=None)
    return coef.astype(np.float64)


def _eval_poly_x(y: float | np.ndarray, coef: np.ndarray) -> np.ndarray:
...
def _eval_poly_dxdy(y: float | np.ndarray, coef: np.ndarray) -> np.ndarray:
    y = np.asarray(y, dtype=np.float64)
    out = np.zeros_like(y, dtype=np.float64)
    for p, a in enumerate(coef):
        if p == 0:
...
def fit_centerline_from_candidates(
    cands: Sequence[Candidate],
    cfg: Cfg,
) -> np.ndarray:
    """
    Weighted fit of x = c(y) from candidate centers.
    """
    if len(cands) == 0:
        return np.array([cfg.image_size / 2.0], dtype=np.float64)

    nodes = sorted(cands, key=lambda c: c.fused_score, reverse=True)[:cfg.centerline_keep_topk]
    ys = np.array([c.center[0] for c in nodes], dtype=np.float64)
    xs = np.array([c.center[1] for c in nodes], dtype=np.float64)

    scores = np.array([c.fused_score for c in nodes], dtype=np.float64)
    smin = float(scores.min())
    smax = float(scores.max())
    if smax <= smin + 1e-8:
        sn = np.ones_like(scores, dtype=np.float64)
    else:
        sn = (scores - smin) / (smax - smin + 1e-8)

    ws = np.exp(cfg.centerline_weight_alpha * sn)
    coef = _polyfit_x_of_y(
        ys=ys,
        xs=xs,
        ws=ws,
        deg=cfg.centerline_poly_deg,
    )
    return coef


def _estimate_nominal_spacing(
    nodes: Sequence[Candidate],
    coef: np.ndarray,
    cfg: Cfg,
) -> float:
    """
    Estimate d_bar on current image.
    Robustly uses near-centerline candidates and consecutive y-gaps.
    """
    if len(nodes) <= 1:
        return 0.5 * (cfg.edge_min_dy + cfg.edge_max_dy)

    ys = np.array([c.center[0] for c in nodes], dtype=np.float64)
    xs = np.array([c.center[1] for c in nodes], dtype=np.float64)
    dist = np.abs(xs - _eval_poly_x(ys, coef))

    order = np.argsort(ys)
    ys = ys[order]
    dist = dist[order]

    dist_thr = float(np.percentile(dist, 60.0)) if len(dist) >= 3 else float(dist.max() + 1e-6)

    gaps = []
    last_y = None
    for y, d in zip(ys, dist):
        if d > dist_thr:
            continue
        if last_y is None:
            last_y = y
            continue
        dy = float(y - last_y)
        if cfg.edge_min_dy <= dy <= cfg.edge_max_dy:
            gaps.append(dy)
        last_y = y

    if len(gaps) == 0:
        all_gaps = []
        ys2 = np.sort(np.array([c.center[0] for c in nodes], dtype=np.float64))
        for i in range(len(ys2) - 1):
            dy = float(ys2[i + 1] - ys2[i])
            if cfg.edge_min_dy <= dy <= cfg.edge_max_dy:
                all_gaps.append(dy)
        if len(all_gaps) == 0:
            return 0.5 * (cfg.edge_min_dy + cfg.edge_max_dy)
        return float(np.median(all_gaps))

    return float(np.median(gaps))


def _node_score_centerline(
    node: Candidate,
    coef: np.ndarray,
    cfg: Cfg,
) -> float:
    """
    U(i;c) = fused_score - centerline distance penalty + lambda_K
    """
    cy, cx = node.center
    x_on_line = float(_eval_poly_x(np.array([cy]), coef)[0])
    d = abs(cx - x_on_line)

    score = (
        node.fused_score
        - cfg.centerline_dist_penalty * d
        + cfg.chain_lambda_K
    )
    return float(score)


def _edge_score_centerline(
    a: Candidate,
    b: Candidate,
    coef: np.ndarray,
    d_bar: float,
    ...
    """
    First-order edge score:
      - spacing penalty
      - gap penalty G(Δ)=max(0, Δ/d_bar - 1)
      - scale / angle penalty
      - centerline tangent consistency
      + lambda_L * Δ
    """
    ya, xa = a.center
    yb, xb = b.center
    ...
    # use centerline tangent to predict lateral movement
    y_mid = 0.5 * (ya + yb)
    slope_mid = float(_eval_poly_dxdy(np.array([y_mid]), coef)[0])
    x_pred = xa + slope_mid * dy
    resid = abs(xb - x_pred)
    if resid > cfg.edge_max_dx:
        return None
    ...
    # G(Δ)=max(0, Δ/d_bar - 1)
    gap_pen = max(0.0, dy / max(d_bar, 1e-6) - 1.0)

    score = (
        - cfg.chain_spacing_lambda * abs(dy - d_bar) / max(d_bar, 1e-6)
        - cfg.chain_gap_lambda * gap_pen
        - cfg.chain_dx_penalty * resid
        - cfg.chain_scale_penalty * abs(math.log(area_ratio + 1e-8))
        - cfg.chain_angle_penalty * (dtheta / 45.0)
        + cfg.chain_lambda_L * dy
    )
    return float(score)


def _second_order_score(
    a: Candidate,
    b: Candidate,
    c: Candidate,
    cfg: Cfg,
    ...
    score = (
        - cfg.chain_ratio_lambda * abs(math.log((dy2 + 1e-6) / (dy1 + 1e-6)))
        - cfg.chain_slope_lambda * abs(r2 - r1)
    )
    return float(score)


def _reconstruct_chain_from_pair(
    nodes: Sequence[Candidate],
    prev: Dict[Tuple[int, int], int],
    end_pair: Tuple[int, int],
) -> List[Candidate]:
    """
    Reconstruct chain from pair-state DP.
    """
    j, k = end_pair
    idxs = [k, j]
    cur = end_pair
    ...
    chain = [nodes[i] for i in idxs]

    # prune overlaps, keep higher fused_score
...
def _select_chain_given_centerline(
    cands: Sequence[Candidate],
    coef: np.ndarray,
    cfg: Cfg,
) -> List[Candidate]:
    """
    Second-order DP on candidates given fitted centerline.
    """
    if len(cands) == 0:
        return []
    ...
    d_bar = _estimate_nominal_spacing(nodes, coef, cfg)

    U = np.array([_node_score_centerline(node, coef, cfg) for node in nodes], dtype=np.float64)

    # best single node fallback
    best_single_idx = int(np.argmax(U))
    best_single_score = float(U[best_single_idx])

    # dp[(j,k)] = best score for chain ending with j -> k
    dp: Dict[Tuple[int, int], float] = {}
    prev: Dict[Tuple[int, int], int] = {}
    length: Dict[Tuple[int, int], int] = {}

    # init length-2 chains
    for j in range(n):
        for k in range(j + 1, n):
            e = _edge_score_centerline(nodes[j], nodes[k], coef, d_bar, cfg)
            if e is None:
                continue
            key = (j, k)
            dp[key] = U[j] + U[k] + e
    ...
    keys_by_second: Dict[int, List[Tuple[int, int]]] = {}
    for key in dp.keys():
        keys_by_second.setdefault(key[1], []).append(key)

    for k in range(n):
        pass  # only here to keep structure clear

    updated = True
    while updated:
        updated = False
        new_items: Dict[Tuple[int, int], Tuple[float, int, int]] = {}

        for (i, j), cur_score in list(dp.items()):
            for k in range(j + 1, n):
                e = _edge_score_centerline(nodes[j], nodes[k], coef, d_bar, cfg)
                if e is None:
                    continue
                s2 = _second_order_score(nodes[i], nodes[j], nodes[k], cfg)
                cand_score = cur_score + U[k] + e + s2
                key2 = (j, k)

                if key2 not in dp and key2 not in new_items:
                    new_items[key2] = (cand_score, i, length[(i, j)] + 1)
                    continue

                old_score = dp[key2] if key2 in dp else new_items[key2][0]
                if cand_score > old_score:
                    new_items[key2] = (cand_score, i, length[(i, j)] + 1)

        if len(new_items) > 0:
            updated = True
            for key2, (score2, prev_i, len2) in new_items.items():
                if key2 not in dp or score2 > dp[key2]:
                    dp[key2] = score2
                    prev[key2] = prev_i
                    length[key2] = len2

    # choose best valid chain
    valid_pairs = [k for k, v in length.items() if v >= cfg.chain_min_len]

    if len(valid_pairs) > 0:
        end_pair = max(valid_pairs, key=lambda key: dp[key])
        best_pair_score = float(dp[end_pair])
        if best_pair_score >= best_single_score:
            return _reconstruct_chain_from_pair(nodes, prev, end_pair)

    # fallback to best length-2 chain
    if len(dp) > 0:
        end_pair = max(dp.keys(), key=lambda key: dp[key])
        if float(dp[end_pair]) >= best_single_score:
            return _reconstruct_chain_from_pair(nodes, prev, end_pair)

    # fallback to best single node
    return [nodes[best_single_idx]]


def select_candidate_chain(cands: Sequence[Candidate], cfg: Cfg) -> List[Candidate]:
    """
    Stable legal-chain selection:
      1) fit centerline x=c(y)
      2) second-order DP with:
         + lambda_K * K
         + lambda_L * (t_iK - t_i1)  [implemented as per-edge +lambda_L * Δy]
         - gap penalty G(Δ)=max(0, Δ/d_bar - 1)
      3) optional one more centerline refinement using selected chain
    """
    if len(cands) == 0:
        return []

    # initial fit on all candidates
    coef = fit_centerline_from_candidates(cands, cfg)
    chain = _select_chain_given_centerline(cands, coef, cfg)

    # refine centerline using selected chain, then select again
    for _ in range(max(0, cfg.centerline_refine_iters - 1)):
        if len(chain) == 0:
            break
        coef = fit_centerline_from_candidates(chain, cfg)
        new_chain = _select_chain_given_centerline(cands, coef, cfg)
        if len(new_chain) == 0:
            break
        chain = new_chain
